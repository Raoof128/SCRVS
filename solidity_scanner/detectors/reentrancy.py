...
class ReentrancyDetector(BaseDetector):
    ...
    def _check_missing_guard(
        self, func: FunctionNode, contract: ContractInfo, file_path: str
    ) -> None:
        ...
        has_external_call = bool(
            re.search(r"\.(call|send|transfer|delegatecall)(\s*\{[^}]*\})?\s*\(", func.body)
        )
    ...
    def _check_cei_violation(
        self, func: FunctionNode, contract: ContractInfo, file_path: str, source_code: str
    ) -> None:
    ...
    def _check_fallback_vectors(
        ...
        if func.visibility in ["public", "external"] and func.is_payable:
            ...
            if has_state_change:
                ...
                has_access_control = bool(re.search(r"require\s*\(.*msg\.sender", func.body))

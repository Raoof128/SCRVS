...
class BadPatternsDetector(BaseDetector):
    ...
    def _check_unprotected_admin(self, func: FunctionNode, file_path: str) -> None:
        ...
        if is_admin_function:
            ...
            has_access_control = any(
                modifier in ["onlyOwner", "onlyAdmin", "onlyRole"] for modifier in func.modifiers
            )

            # Check for require() checks
            has_require = bool(re.search(r"require\s*\(.*msg\.sender", func.body))
    ...
    def _check_missing_events(
        ...
        if has_state_change:
            ...
            has_event = bool(re.search(r"emit\s+\w+\s*\(", func.body))

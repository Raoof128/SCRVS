...
class SolidityParser:
        ...
        for i, line in enumerate(contract_lines, start=contract.line_start):
            ...
            if match:
                ...
                if paren_end > 0:
                    ...
                    after_sig = re.sub(r'\b(public|private|internal|external|payable|view|pure|returns)\b', '', after_sig)
                    ...
                    for mod in modifier_matches:
                        if mod and mod not in ['function', 'public', 'private', 'internal', 'external', 'payable', 'view', 'pure', 'returns']:
    ...
    def get_state_writes(self, function_body: str, state_variables: List[StateVariable]) -> List[Tuple[str, int]]:
        """
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            for match in matches:
                ...
                if (start_pos == 0 or line_before_comment[start_pos - 1] in ' =,;()[]{}"\''):
                    if (end_pos >= len(line_before_comment) or 
                        line_before_comment[end_pos] in ' ,;()[]{}"\''):
